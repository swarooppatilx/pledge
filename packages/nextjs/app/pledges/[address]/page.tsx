...
import { FounderShareCard, IcoLockBadge, ProjectAvatar, SimulatedYieldTicker, UniswapWidget } from "~~/components/ui";
import {
  useBuyTreasuryStock,
  useCancelCampaign,
  useClaimRewards,
...
import { PledgeStatus, calculateProgress, isDeadlinePassed, timeRemaining } from "~~/types/pledge";
...
const PledgeDetailPage: NextPage = () => {
  ...
  } = useFounderShare(pledge?.token as `0x${string}`, pledge?.creator as `0x${string}`);
  ...
  const { cancel, isPending: isCancelling } = useCancelCampaign();
  ...
  const isCancelled = pledge.status === PledgeStatus.Cancelled;
  const isCreator = connectedAddress?.toLowerCase() === pledge.creator.toLowerCase();
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8">
          <div className="flex items-start gap-4">
            <ProjectAvatar address={pledgeAddress} name={pledge.name} size="xl" />
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                {isFailed && <span className="tag bg-[rgba(255,67,67,0.1)] text-[#FF4343]">Failed</span>}
              </div>
              ...
              <div className="card-pledge">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-[#1B1B1B] rounded-xl p-4 border border-[#222222]">
                    ...
                    <div className="text-h2 font-mono text-white">
                      {icoPrice > 0n ? formatEthValue(icoPrice, 8) : "0"}
                    </div>
                    <div className="text-label text-muted">ETH per share</div>
                  </div>
                  <div className="bg-[#1B1B1B] rounded-xl p-4 border border-[#222222]">
                    <div className="text-label text-muted mb-1">Floor Price</div>
                    <div className="text-h2 font-mono text-[#27AE60]">
                      {floorPrice > 0n ? formatEthValue(floorPrice, 8) : "0"}
                    </div>
                    <div className="text-label text-muted">ETH per share</div>
                  </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-4">
                ...
                <div className="bg-[#1B1B1B] rounded-xl p-4 mb-4 border border-[#FF007A]/20">
                  <div className="flex justify-between text-body">
                    ...
                    <span className="font-mono text-white">
                      {formatEthValue(pledge.fundingGoal - pledge.totalRaised)} ETH
                    </span>
                  </div>
                  {icoPrice > 0n && (
            ...
            {isActive && (
              <>
                ...
                <UniswapWidget tokenAddress={pledge.token} tokenSymbol={pledge.ticker} isActive={isActive} />
                  ...
                  {holderInfo && (
                    <div className="bg-[#1B1B1B] rounded-xl p-4 mb-4 border border-[#27AE60]/30">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono text-[#27AE60] text-h2">
                          {formatEthValue(holderInfo.pendingRewards)} ETH
                        </span>
                      </div>
                    </div>
                  )}
                ...
                <div className="card-pledge border-[#F2994A]/30 relative overflow-hidden">
                  ...
                  {holderInfo && holderInfo.shareBalance > 0n && (
                    <p className="text-label text-muted mb-3">
                      You hold:{" "}
                      <span className="font-mono text-white">
                        {Number(holderInfo.shareBalance / BigInt(1e18)).toLocaleString()}
                      </span>{" "}
                      shares
                    </p>
                  )}
                  ...
                  {redeemAmount && pledge.circulatingSupply > 0n && (
                    <p className="text-label text-[#27AE60] mt-3">
                      ≈{" "}
                      {formatEthValue(
                        (parseEther(redeemAmount || "0") * pledge.vaultBalance) / pledge.circulatingSupply,
                      )}{" "}
                      ETH
                    </p>
                  )}
                  ...
                  <div className="card-pledge">
                    ...
                    <div className="bg-[#1B1B1B] rounded-xl p-4 mb-4 border border-[#222222]">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono text-white">
                          {Number(pledge.treasuryShares / BigInt(1e18)).toLocaleString()} shares
                        </span>
                      </div>
                    </div>
            ...
            {/* Cancelled State */}
            {isCancelled && (
              <div className="card-pledge border-[#9B9B9B]/30">
                <h2 className="text-h2 text-[#9B9B9B] mb-3">Campaign Cancelled</h2>
                <p className="text-body text-muted mb-4">
                  This pledge was cancelled by the creator. Contributors can claim refunds.
                </p>
                {holderInfo && holderInfo.contribution > 0n && (
                  <div className="bg-[#1B1B1B] rounded-xl p-4 mb-4 border border-[#9B9B9B]/20">
                    <div className="flex justify-between items-center">
                      <span className="text-label text-muted">Your contribution</span>
                      <span className="font-mono text-white">{formatEthValue(holderInfo.contribution)} ETH</span>
                    </div>
                ...
                <button
                  className="btn-brand w-full bg-[#9B9B9B] hover:bg-[#9B9B9B]/80"
                  onClick={handleRefund}
                  disabled={isRefunding || !connectedAddress || !holderInfo || holderInfo.contribution === 0n}
                >
                  {isRefunding ? <span className="loading loading-spinner"></span> : "Claim Refund"}
            ...
            {/* Cancel Campaign - Creator Only (during funding) */}
            {isFunding && isCreator && (
              <div className="card-pledge border-[#F2994A]/30">
                <h2 className="text-h2 text-[#F2994A] mb-2">Cancel Campaign</h2>
                <p className="text-label text-muted mb-4">
                  As the creator, you can cancel this campaign. All contributors will be able to claim refunds.
                </p>
                <button
                  className="btn-brand w-full bg-[#F2994A] hover:bg-[#F2994A]/80"
                  onClick={handleCancelCampaign}
                  disabled={isCancelling}
                >
                  {isCancelling ? <span className="loading loading-spinner"></span> : "Cancel Campaign"}
                </button>
              </div>
            )}
                  ...
                  <div className="mt-4 p-4 bg-[#1B1B1B] rounded-xl border border-[#222222]">
                    ...
                    {holderInfo.redeemableValue + holderInfo.pendingRewards > holderInfo.contribution ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-[#EB5757]/20 text-[#EB5757] rounded-lg text-label">
                        -
                        {formatEthValue(
                          holderInfo.contribution - holderInfo.redeemableValue - holderInfo.pendingRewards,
                        )}{" "}
                        ETH loss
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-[#5E5E5E]/20 text-[#9B9B9B] rounded-lg text-label">
                        Break even
                      </span>
                    )}
                  </div>
